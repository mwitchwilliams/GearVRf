 ...
 * code.
 */
public class X3DLayeredShader extends GVRShaderTemplate
{
    ...
    public X3DLayeredShader(GVRContext gvrcontext)
    {
         super( "float4 ambient_color; float4 diffuse_color; float4 specular_color; float4 emissive_color;" +
                ...
                "float specular_exponent; float line_width; float pad; mat3 texture_matrix",
                ...
                "float3 a_position float2 a_texcoord float2 a_texcoord1 float2 a_texcoord2 float2 a_texcoord3 float3 a_normal",
                GLSLESVersion.VULKAN);

        if (fragTemplate == null)
        {
            Context context = gvrcontext.getContext();
            fragTemplate = TextFile.readTextFile(context, org.gearvrf.R.raw.fragment_template_multitex);
            vtxTemplate = TextFile.readTextFile(context, org.gearvrf.R.raw.vertex_template_multitex);
            surfaceShader = TextFile.readTextFile(context, org.gearvrf.x3d.R.raw.x3d_surface_multitex);
            vtxShader = TextFile.readTextFile(context, org.gearvrf.R.raw.pos_norm_multitex) +
        ...
        setSegment("VertexTemplate", vtxTemplate.replace("@MATRIX_UNIFORMS", "@MATRIX_UNIFORMS\n@MATERIAL_UNIFORMS\n"));
        setSegment("FragmentSurface", surfaceShader);
        setSegment("FragmentAddLight", addLight);
        setSegment("VertexSkinShader", null);
        setSegment("VertexShader", vtxShader);
        setSegment("VertexNormalShader", null);
